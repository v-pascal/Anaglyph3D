...
public class Connectivity {
    ...
    public boolean mListenDevice = false; // false: Try to connect (master), true: Listen (slave)
    ...
    private boolean processRequest(String request) {
        ...
        reply.handler = getHandlerInstance(request.charAt(0));
        if (reply.handler == null) {

            Logs.add(Logs.Type.E, "Unexpected request handler received");
